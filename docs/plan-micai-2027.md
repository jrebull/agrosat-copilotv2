...
El punto 2 está **cerrado**: el autor de correspondencia es Arthur Jafed Zizumbo-Velasco, decidido el 3 de septiembre, y ya va impreso. El camera-ready existe (`make micai-pdf-cr`, quince páginas, ORCID, afiliación, créditos y Disclosure of Interests) y lleva impresa una nota al pie Sale del mismo `main.tex` mediante un testigo en disco, y está comprobado que **el cuerpo es byte a byte el mismo** que el de la versión anónima. Queda el punto 5, el empaquetado `.zip`.
