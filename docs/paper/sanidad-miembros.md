...
**1. Que la métrica declarada describe los pesos que el arnés carga.** `unet` y `anysat` guardan su
métrica en un fichero **distinto** del que el registro carga: el registro apunta a `*_pastis.pt`, un
`state_dict` pelado sin métricas, y las métricas viven en `*_ckpt.pt`. Comparar la métrica de un
fichero con el volcado de otro solo vale si los pesos son los mismos, así que se comprobó tensor a
tensor: **380 de 380 idénticos en `unet`, 501 de 501 en `anysat`**. Sin esa comprobación, esas dos
filas serían una suposición con formato de tabla.

**2. Que ningún checkpoint registra sus folds de entrenamiento.** Se declara que **el registro no
existe**, en vez de deducirlo. El volcado usa `(1,2,3)` para las estadísticas de normalización, pero
eso es una elección del arnés y no una constancia de cómo se entrenó cada modelo.
