...
3. **La delegación es condicional.** Solo se usan subagentes cuando el usuario lo pide, el arnés los
   ofrece y el trabajo puede dividirse sin escrituras conflictivas. Las plantillas existentes son
   ayudas de despacho, no una obligación universal.
4. **Graphify es opcional.** Existen targets y configuración, pero su presencia no sustituye `rg`,
   imports, tests ni `git log`, y su ausencia no bloquea una US. Nunca se afirma que un grafo fue
   actualizado si el comando no corrió.
5. **Engram degrada de forma segura.** La configuración actual contiene una lista de permisos, pero
   `enabledPlugins` está vacío y no existen `make memory-sync` ni `make memory-import`. Por tanto,
   Engram solo está activo donde cada desarrollador lo haya instalado localmente. Su ausencia
...
6. **No hay sincronización automática de memoria.** `engram sync` escribe `.engram/` y puede incluir
   ...
   -f .engram` ni se habilita nube sin aceptar
   [`ADR-015`](../decisions/ADR-015-engram-memoria-compartida.md).
7. **Los comandos se comprueban antes de exigirlos.** Los gates MICAI disponibles se obtienen del
   `Makefile` y del job `quality-gates`; no se mantiene una segunda lista manual que pueda quedar
   obsoleta. La lista operativa de comandos está en
