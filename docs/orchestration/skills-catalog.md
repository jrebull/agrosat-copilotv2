...
> Catálogo de las 31 skills `agrosat-*`. Resumen ejecutivo en [`AGENTS.md`](../../AGENTS.md). Detalle de cada skill en su `SKILL.md`.

Las skills viven en `.claude/skills/<nombre>/SKILL.md` con frontmatter YAML (`name`, `description`, `allowed-tools`). Claude las carga automáticamente por `description` o por invocación manual `/<nombre>`. La skill de la VM Azure H100 **se retiró** el 4-sep-2026 (la máquina ya no existe; AGENTS.md raíz §Descartados).
...
| `agrosat-llm-finetuning` | Gemma 4 LoRA y serving de LLM: **FUTURE** (ADR-011), sin hardware y fuera del alcance del artículo |
...
| `agrosat-terraform` | Terraform GCP `dev` (único entorno vivo); el módulo Azure es código histórico |
| `agrosat-gcp-services` | Cloud Run, Cloud SQL, GCS, Pub/Sub, Vertex AI, Secret Manager, IAM |
| `agrosat-finops` | Costos GCP: scale-to-zero, Cloud SQL detenida, GPU L4 spot por US con tope en el spec |
...
| Skill | Descripción |
|---|---|
| `agrosat-git-workflow` | Ramas `feature/E{epic}-US-XXX-{slug}` sobre `main`, Conventional Commits con scope de épica, PR con memoria sincronizada, cierre de US por el loop |
| `agrosat-engram-memory` | Memoria de desarrollo del equipo: SQLite local + chunks en `.engram/` que viajan en cada PR, proyecto canónico `agrosat-copilotv2` |
