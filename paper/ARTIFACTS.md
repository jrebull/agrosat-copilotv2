...
**Sellado el**: 2026-09-04 · **Commit de sellado**: `0f91a4d`
...
| Fase 3: figura de la frontera (PDF) | `reports/paper_micai/fase3/frontera.pdf` | `267902e7f2df8645c5803b0a36e5aa77` | 19334 | sin seguimiento en git (`*.pdf` global) | OBSOLETO | Formato que incluye LaTeX. Hereda el estado de su SVG y de los insumos de fase 3. |
...
| Fase 4: figura de la replica (PDF) | `reports/paper_micai/fase4/replica.pdf` | `13ab4c6f7675059f26115d1973adeb29` | 18205 | sin seguimiento en git (`*.pdf` global) | OBSOLETO | Formato que incluye LaTeX. Hereda el estado de su SVG. |
...
| Manuscrito nuevo: figura del soporte por clase, espanol | `reports/micai2027/figuras/soporte-es.svg` | `6b0c76ee9cbf4b27172a7c1e88205356` | 69160 | `0f91a4d` | SELLADO | Primera figura del manuscrito NUEVO. Ancho fisico exacto de `\textwidth` de LNCS (4,8031 in), sin escala de colocacion; tipografia impresa por encima de 8 pt; clases de cola distinguidas por color **y** trama. El banco se rotula PASTIS, no PASTIS-R. Reproducible byte a byte: dos generaciones seguidas dan el mismo MD5. Sus dos insumos son SELLADO, asi que es citable. **Resellada**: la primera version imprimia los dos titulos en una linea, el del panel derecho invadia al izquierdo y se salia del lienzo -«60 000 par» cortado a media palabra-. El contrato de figuras medía ancho, tipografia y canales, y ninguna de las tres ve un texto recortado; ahora lo comprueba. |
| Manuscrito nuevo: figura del soporte por clase, espanol (PDF) | `reports/micai2027/figuras/soporte-es.pdf` | `861b182db4ab496a58012e93afb6fd96` | 21376 | sin seguimiento en git (`*.pdf` global) | SELLADO | **El fichero que de verdad entra en el manuscrito**: `\includegraphics` toma el PDF, no el SVG. Ancho de pagina medido 345,827 pt = `\textwidth` de LNCS, factor de escala 1. |
| Manuscrito nuevo: figura del soporte por clase, ingles | `reports/micai2027/figuras/soporte-en.svg` | `e3a993c5790cbf50ad811c5da901c8cc` | 68277 | `0f91a4d` | SELLADO | La misma figura con los rotulos en ingles, que es la lengua del envio. Mismos insumos, mismo contrato fisico, misma reproducibilidad. El `.pdf` de cada una es la misma figura en otro formato y no lleva fila propia: el ledger sella un representante por figura. |
| Manuscrito nuevo: figura del soporte por clase, ingles (PDF) | `reports/micai2027/figuras/soporte-en.pdf` | `a0a6f636d51f0af5518788e72d2fee22` | 21101 | sin seguimiento en git (`*.pdf` global) | SELLADO | El mismo, con rotulos en ingles. Mismo contrato fisico y misma reproducibilidad byte a byte. |
...
| Fase 6: figura de parcelas reales del fold retenido (PDF) | `reports/paper_micai/figuras/parcelas.pdf` | `c68dee32491a17fd8d124231a6b47dc2` | 198952 | sin seguimiento en git (`*.pdf` global) | SELLADO | Formato que incluye LaTeX. Tres parches en color natural y su anotacion agronomica. |
...
| Fase 6: figura del soporte por clase de los dos bancos (PDF) | `reports/paper_micai/figuras/soporte.pdf` | `5e439fab930dbc30d4f75f50609eb3ba` | 20976 | sin seguimiento en git (`*.pdf` global) | SELLADO | Formato que incluye LaTeX. Regenerado con el rotulo PASTIS corregido, igual que su SVG. |
...
| Fase 6: figura de composicion de leyendas (PDF) | `reports/paper_micai/figuras/leyendas.pdf` | `1711a54c2f64be5c0439f69787aad24a` | 18014 | sin seguimiento en git (`*.pdf` global) | OBSOLETO | Formato que incluye LaTeX. Hereda el estado de su SVG. |
...
| Fase 6: figura de la frontera en cobertura (PDF) | `reports/paper_micai/figuras/cobertura.pdf` | `6d552918d0a8dac32f5cb3b2dd57b830` | 23048 | sin seguimiento en git (`*.pdf` global) | OBSOLETO | Formato que incluye LaTeX. Hereda el estado de su SVG. Su rotulo aun dice PASTIS-R: no se regenera hasta que se regeneren sus insumos. |
