...
demo-vm:  ## HISTORICO: demo + tunel al Qwen on-prem; la VM del sponsor se perdio, no se reactiva
...
check: lint secrets-scan i18n-check guides-check  ## suite local previa a PR (reemplaza pre-commit)
...
graph-check:  ## Falla si graphify-out/graph.json no describe HEAD (o no existe)
	$(HARNESS_PY) scripts/harness_status.py --graph-check
...
harness-status:  ## Que hay en vuelo: rama, grafo, memoria pendiente, US abiertas, espejos, plan
	$(HARNESS_PY) scripts/harness_status.py

guides-sync:  ## Copia AGENTS.md sobre CLAUDE.md en todos los pares (se edita AGENTS.md)
	$(HARNESS_PY) scripts/harness_check.py --sync

guides-check:  ## Falla si algun par AGENTS.md / CLAUDE.md difiere
	$(HARNESS_PY) scripts/harness_check.py --guides-only
